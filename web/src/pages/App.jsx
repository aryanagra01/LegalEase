...
function safeUUID() {
  try { const g = (typeof window !== 'undefined' ? window : self); if (g?.crypto?.randomUUID) return g.crypto.randomUUID() } catch {}
  return 'sid_' + Math.random().toString(36).slice(2) + Date.now().toString(36)
}

function Badge({children}) { return <span className="text-xs bg-gray-200 rounded px-2 py-0.5">{children}</span> }
...
export default function App(){
  ...
  useEffect(()=>{ if (scrollRef.current) scrollRef.current.scrollTop = scrollRef.current.scrollHeight }, [log, question])

  const legalBanner = (brief.domain?.toLowerCase().includes("legal") || brief.jurisdiction)

  async function callCoach(userText, opts={}){
    const body = { sessionId, brief, user_input: userText, forceFinal: !!opts.forceFinal }
  ...
  async function send(userText){
    ...
    const resp = await callCoach(userText)
    if (!resp || !resp.ok){ setLog(l => [...l, { role:'coach', content: 'Connection issue. Is the server on port 3001 running?' }]); return }
    const data = await resp.json(); handleCoach(data)
  }

  async function skipAhead(){
    setLog(l => [...l, { role:'user', content:'[Skip ahead & draft prompt]' }])
    const resp = await callCoach('', { forceFinal:true })
    if (!resp || !resp.ok){ setLog(l => [...l, { role:'coach', content: 'Connection issue. Is the server on port 3001 running?' }]); return }
    const data = await resp.json(); handleCoach(data)
  }

  function handleCoach(data){
    if (data.updated_brief) setBrief(data.updated_brief)
    if (typeof data.progress === 'number') setProgress(data.progress)
    if (data._field_status) setStatusMap(data._field_status)
    if (data.phase) setPhase(data.phase)
    if (data.phase === 'final') {
      setGold(data.gold_prompt || ""); setVerify(data.verify_list || [])
      setQuestion("All set."); setWhy("You can copy the Gold Prompt on the right.")
...
Why this matters: ${data.why_it_matters}` }])
  ...
  function copyGold(){ navigator.clipboard.writeText(gold || "").then(()=>alert("Gold prompt copied")) }
  function resetAll(){
    ...
    setLog([]); setInput(""); setStatusMap({domain:false,goal:false,facts:false,obstacles:false,resources:false,constraints:false,examples:false,audience:false,desired_output:false,success_criteria:false,jurisdiction:false})
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 space-y-4">
      ...
      <div className="h-2 bg-gray-200 rounded"><div className="h-2 bg-black rounded" style={progressStyle} /></div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="md:col-span-1"><Checklist statusMap={statusMap} /></div>
        ...
        <div className="md:col-span-1 space-y-3">
          <div className="bg-white border rounded-xl p-3 h-[75vh] flex flex-col">
            <div className="flex items-center justify-between mb-2">
              ...
              {phase !== 'final' && <button onClick={skipAhead} className="px-3 py-1.5 rounded-md border text-xs">Skip ahead & Draft Prompt</button>}
            </div>

            <div ref={scrollRef} className="flex-1 overflow-y-auto pr-1 space-y-2 text-sm">
              {log.map((m,i)=>(
                <div key={i} className="flex items-start gap-2">
                  {m.role==='coach' ? (<img src="/ms-legalease.png" alt="Ms LegalEase" className="h-6 w-6 rounded-full border object-cover mt-0.5"/>) :
              ...
              {phase === 'confirm' && (
                <div className="flex gap-2 mt-2">
                  <button onClick={()=>send('yes')} className="px-3 py-1.5 rounded-md bg-black text-white text-sm">Yes, looks good</button>
                  <button onClick={()=>send('no')} className="px-3 py-1.5 rounded-md border text-sm">No, change something</button>
                  <button onClick={skipAhead} className="px-3 py-1.5 rounded-md border text-sm">Skip ahead & Draft Prompt</button>
                </div>
              )}
            </div>

            <div className="mt-3 flex gap-2">
              <input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=> e.key==='Enter' && input.trim() && send(input)} placeholder={phase==='gather' ? "Type your answer or initial problem…" : "Respond here (e.g., 'yes' to confirm)"} className="flex-1 border rounded-lg px-3 py-2" />
              <button onClick={()=> input.trim() && send(input)} className="px-4 py-2 rounded-lg bg-black text-white">Send</button>
            </div>
          </div>
        </div>
        ...
        <div className="md:col-span-1">
          <div className="bg-white border rounded-xl p-3 h-[75vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-2">
              <h2 className="font-medium">Gold Prompt</h2>
              {phase !== 'final' && <button onClick={skipAhead} className="px-3 py-1.5 rounded-md border text-xs">Skip ahead & Draft Prompt</button>}
            </div>
            {phase === 'final' ? (
              <>
                <pre className="text-sm whitespace-pre-wrap">{gold}</pre>
                {verify?.length>0 && (<><h3 className="font-medium mt-3">Verify before using:</h3><ul className="list-disc list-inside text-sm">{verify.map((v,i)=><li key={i}>{v}</li>)}</ul></>)}
                <button onClick={copyGold} className="mt-3 px-3 py-1.5 border rounded-md text-sm">Copy prompt</button>
              </>
            ) : (<div className="text-sm text-gray-500">You can skip ahead at any time to draft the best possible prompt from the info so far.</div>)}
          </div>
        </div>
      </div>
    </div>
